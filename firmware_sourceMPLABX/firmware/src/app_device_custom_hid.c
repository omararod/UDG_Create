        ...
        switch(ReceivedDataBuffer[0])				//Look at the data the host sent, to see what kind of application specific command it sent.
        {
            case COMMAND_TOGGLE_LED:  //Toggle LEDs command
              //  LED_Toggle(LED_USB_DEVICE_HID_CUSTOM);
                break;
            case ROBOT_ON_ROBOT_CHARGING:  //Get push button state
            ...
            case GET_SENSORS:	//Read POT command.  Uses ADC to measure an analog voltage on one of the ANxx I/O pins, and returns the result to the host
                {
                    uint16_t pot;
